...
#include "ui_scrollablegraphicdisplay3.h"
#include <QVBoxLayout>
#include <QSpacerItem>
...
ScrollableGraphicDisplay3::ScrollableGraphicDisplay3(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ScrollableGraphicDisplay3)
{
    ui->setupUi(this);
...
ScrollableGraphicDisplay3::~ScrollableGraphicDisplay3()
{
    delete ui;
}
AllRoutes *ScrollableGraphicDisplay3::getAllRoutes() const
{
    return allRoutes;
}

void ScrollableGraphicDisplay3::setAllRoutes(AllRoutes *value)
{
    allRoutes = value;
}

void ScrollableGraphicDisplay3::setupUI(){
    ...
    */
    ...
    for(int i = 0 ; i < allRoutes->getRoutes().size(); i++){
        ...
        DisplayGLGraphics3RouteWidget *displayGLGraphics3RouteWidget = new DisplayGLGraphics3RouteWidget((allRoutes->getRoutes().at(i)),originalRoute,glWindowSize, heightPerRow);
        displayGLGraphics3RouteWidget->setSizePolicy(QSizePolicy::Expanding,QSizePolicy::Fixed);
        displayGLGraphics3RouteWidget->setObjectName(widgetName + QString::number(i));
        wrapperVerticalLayout->addWidget(displayGLGraphics3RouteWidget);
...
void ScrollableGraphicDisplay3::on_allRoutesSelectCheckBox_clicked()
{
...
void ScrollableGraphicDisplay3::on_allRoutesGenerateSSPushButton_clicked()
{

    bool atLeastAFileIsSaved = false;
    QList<QCheckBox*> routeCheckBoxes = findChildren<QCheckBox *>("selectRouteCheckBox",Qt::FindChildrenRecursively);

    QList<GLGraphics3RouteWidget *> glWidgets = findChildren<GLGraphics3RouteWidget *>("openglWindow");
    ...
    for(int i = 0 ; i < glWidgets.size(); i++){
        if(routeCheckBoxes.at(i)->isChecked()){
          ...
          frameBuffer.save( saveDir + "Graphic_V3_" + glWidgets.at(i)->route->getRouteName() + ".png");
...
int ScrollableGraphicDisplay3::calculateTotalSubRouteRows(SubRoute &subRoute, int subRouteIndex, SubRoute &originalSubRoute){
...
std::vector<FlightContainer> ScrollableGraphicDisplay3::calculateCancelledFlights(SubRoute &originalSubRoute, SubRoute &tempSubRoute){
