...
#include "ui_scrollablegraphicdisplay1.h"
#include <QVBoxLayout>
#include <QSpacerItem>
...
ScrollableGraphicDisplay1::ScrollableGraphicDisplay1(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ScrollableGraphicDisplay1)
{
    ui->setupUi(this);
...
ScrollableGraphicDisplay1::~ScrollableGraphicDisplay1()
{
    delete ui;
}
...
AllRoutes *ScrollableGraphicDisplay1::getAllRoutes() const
{
    return allRoutes;
}

void ScrollableGraphicDisplay1::setAllRoutes(AllRoutes *value)
{
    allRoutes = value;
}

void ScrollableGraphicDisplay1::setupUI(){
    ...
    */
    ...
    for(int i = 0 ; i < allRoutes->getRoutes().size(); i++){
        ...
        DisplayGLGraphics1RouteWidget *displayGLGraphics1RouteWidget = new DisplayGLGraphics1RouteWidget((allRoutes->getRoutes().at(i)),originalRoute,glWindowSize, heightPerRow);
        displayGLGraphics1RouteWidget->setSizePolicy(QSizePolicy::Expanding,QSizePolicy::Fixed);
        displayGLGraphics1RouteWidget->setObjectName(widgetName + QString::number(i));
        wrapperVerticalLayout->addWidget(displayGLGraphics1RouteWidget);
...
void ScrollableGraphicDisplay1::on_allRoutesSelectCheckBox_clicked()
{
...
void ScrollableGraphicDisplay1::on_allRoutesGenerateSSPushButton_clicked()
{

    bool atLeastAFileIsSaved = false;
    QList<QCheckBox*> routeCheckBoxes = findChildren<QCheckBox *>("selectRouteCheckBox",Qt::FindChildrenRecursively);

    QList<GLGraphics1RouteWidget *> glWidgets = findChildren<GLGraphics1RouteWidget *>("openglWindow");
    ...
    for(int i = 0 ; i < glWidgets.size(); i++){
        if(routeCheckBoxes.at(i)->isChecked()){
          ...
          frameBuffer.save( saveDir + "Graphic_V1_" + glWidgets.at(i)->route->getRouteName() + ".png");
...
int ScrollableGraphicDisplay1::calculateTotalSubRouteRows(SubRoute &subRoute, int subRouteIndex, SubRoute &originalSubRoute){
...
std::vector<FlightContainer> ScrollableGraphicDisplay1::calculateCancelledFlights(SubRoute &originalSubRoute, SubRoute &tempSubRoute){
